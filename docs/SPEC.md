...
- `output` — where to copy the finished application image after every
  successful `gea build`, `gea flash`, `gea run` or `gea ota`, relative to the
  app. `--output <file>` on the command line overrides it, relative to the
  working directory. The build tree under `.gea/build` stays where it is; this
  is the one file people hand on or upload.
