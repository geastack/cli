...
export function validateApp(app) {
  ...
  for (const [target, config] of Object.entries(app.targetConfig)) {
    ...
    const checkPartitions = (table, field) => {
      if (typeof table === 'string' && !exists(path.join(app.root, table))) {
        errors.push(`${where(field)} does not exist: ${table}`)
        return
      }
      if (!table || typeof table !== 'object') return
      for (const [name, partition] of Object.entries(table)) {
        if (!partition.type) errors.push(`${where(field)}.${name} needs a type`)
        if (!partition.size) errors.push(`${where(field)}.${name} needs a size`)
      }
    }
    checkPartitions(config.partitions, 'partitions')
    for (const [target, table] of Object.entries(config.partitionsByTarget)) {
      checkPartitions(table, `partitionsByTarget.${target}`)
    }
  }
...
// The app's directory, absolute. Consumers (CMake, the apple and geaos build
// scripts) take it as given; nothing joins it onto anything.
...
function emptyTargetConfig() {
  return { ldFragments: [], componentDirs: [], componentRequires: [], linkOptions: [], embedFiles: {}, partitions: null, partitionsByTarget: {}, sdkconfig: '', prebuild: '', output: '' }
}
...
// One app, boards with different flash. The app's needs are the same -- the same
// slots holding the same things -- but a table that runs past the end of a
// smaller chip cannot be written to it, and the app is the only party that knows
// what its slots are for. Keyed by target id rather than board alias: an alias
// is one machine's name for a board, a target id is the board.
function normalizePartitionsByTarget(raw) {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return {}
  const byTarget = {}
  for (const [target, table] of Object.entries(raw)) {
    const normalized = normalizePartitions(table)
    if (normalized) byTarget[target] = normalized
  }
  return byTarget
}
...
function normalizeTargetConfig(raw) {
  ...
  for (const [name, value] of Object.entries(raw)) {
    ...
    configs[name] = {
      ...
      partitionsByTarget: normalizePartitionsByTarget(value.partitionsByTarget),
      ...
      output: typeof value.output === 'string' && value.output.trim() ? normalizeManifestRelativePath(value.output) : ''
    }
  }
  return configs
}
