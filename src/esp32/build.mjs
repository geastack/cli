...
export function applySdkconfigPolicy(sdkconfig, { selection, app, capabilities, bleOta, partitionCsv, boardConsole, boardPsram }) {
  ...
  // USB-UART bridge cannot use the base's native USB-Serial/JTAG console: the
  // firmware would run correctly and log into a port nobody is connected to,
  // which is indistinguishable from a board that never booted.
  if (boardConsole) {
    for (const key of ['CONFIG_ESP_CONSOLE_UART_DEFAULT', 'CONFIG_ESP_CONSOLE_USB_SERIAL_JTAG', 'CONFIG_ESP_CONSOLE_USB_CDC', 'CONFIG_ESP_CONSOLE_NONE']) {
      unset(key)
    }
    if (boardConsole.transport === 'uart') {
      board('CONFIG_ESP_CONSOLE_UART_DEFAULT', 'y')
      board('CONFIG_ESP_CONSOLE_UART', 'y')
      board('CONFIG_ESP_CONSOLE_UART_NUM', String(boardConsole.uart))
      board('CONFIG_ESP_CONSOLE_UART_BAUDRATE', String(boardConsole.baud || 115200))
    } else if (boardConsole.transport === 'usb-serial-jtag') {
      board('CONFIG_ESP_CONSOLE_USB_SERIAL_JTAG', 'y')
      board('CONFIG_ESP_CONSOLE_USB_SERIAL_JTAG_ENABLED', 'y')
      board('CONFIG_ESP_CONSOLE_UART_NUM', '-1')
    } else {
      board('CONFIG_ESP_CONSOLE_NONE', 'y')
      board('CONFIG_ESP_CONSOLE_UART_NUM', '-1')
    }
  }

  // PSRAM wiring follows the board. Inheriting a base's octal mode onto a
  // quad-wired module does not fail the build or the boot: it brings PSRAM up
  // wrong, and the damage shows as corruption under load far from here.
  if (boardPsram) {
    if (boardPsram.mode === 'none') {
      unset('CONFIG_SPIRAM')
      unset('CONFIG_SPIRAM_MODE_OCT')
      unset('CONFIG_SPIRAM_MODE_QUAD')
    } else {
      board('CONFIG_SPIRAM', 'y')
      if (boardPsram.mode === 'octal') {
        unset('CONFIG_SPIRAM_MODE_QUAD')
        board('CONFIG_SPIRAM_MODE_OCT', 'y')
      } else {
        unset('CONFIG_SPIRAM_MODE_OCT')
        board('CONFIG_SPIRAM_MODE_QUAD', 'y')
      }
      if (boardPsram.speed) {
        for (const mhz of [40, 80, 120]) {
          if (mhz !== boardPsram.speed) unset(`CONFIG_SPIRAM_SPEED_${mhz}M`)
        }
        board(`CONFIG_SPIRAM_SPEED_${boardPsram.speed}M`, 'y')
        board('CONFIG_SPIRAM_SPEED', String(boardPsram.speed))
      }
    }
  }

  // Flash size follows the board, not the base target it extends: a composed
  // board declares its own, and without this a 16MB module would inherit its
  // base's 32MB and build a bootloader that maps flash it does not have.
  // Written through `board()` so an app may still overrule it.
  const flashSizeMb = /^(\d+)MB$/i.exec(selection.flashSize || '')?.[1]
  if (flashSizeMb) {
    for (const size of ['2', '4', '8', '16', '32', '64', '128']) {
      if (size !== flashSizeMb) unset(`CONFIG_ESPTOOLPY_FLASHSIZE_${size}MB`)
    }
    board(`CONFIG_ESPTOOLPY_FLASHSIZE_${flashSizeMb}MB`, 'y')
    board('CONFIG_ESPTOOLPY_FLASHSIZE', `"${flashSizeMb}MB"`)
  }
  ...
  // C++ exceptions are a requirement of the compiler backend, not a board
  // preference, which is why this is `set` rather than `board`. geatsc emits
  // real `throw`/`catch` for a program's own throw sites
  // (`compiler/src/targets/cpp/emit-exceptions.ts`), and its runtime header
  // contains typed catch handlers unconditionally -- the regex cache's
  // std::regex_error translation, and a `throw std::length_error` on string
  // concatenation overflow. ESP-IDF defaults to -fno-exceptions, under which
  // gcc rejects those handlers outright, so NO geatsc app compiles at all:
  // `error: exception handling disabled, use '-fexceptions' to enable`, before
  // anything board-specific is even reached.
  //
  // Three boards used to state this in their own sdkconfig.defaults
  // (amoled-2.06, amoled-2.41, papers3) and the other eleven did not, which is
  // the whole reason those eleven could not build any app. A per-board answer
  // to a question no board gets a say in is a rule that has to be remembered
  // once per board; stating it here is the CLI answering for the compiler it
  // invokes. A board or app that asks for `n` is asking for a build that
  // cannot link, so this overrules it.
  set('CONFIG_COMPILER_CXX_EXCEPTIONS', 'y')
  // The emergency pool is heap reserved up front so a throw can still allocate
  // its exception object when the heap is exhausted. 0 -- what the three boards
  // that already worked chose -- means nothing is reserved at rest and the
  // first throw allocates ~500 B normally. An app that must survive throwing
  // under heap exhaustion can raise it in its own defaults.
  board('CONFIG_COMPILER_CXX_EXCEPTIONS_EMG_POOL_SIZE', '0')
...
export function prepareEsp32Build({ ctx, selection, app = null, env = ctx.env || process.env, log = () => {}, bleOta = false, dryRun = false }) {
  ...
  if (selection.targetDefinition) {
    ...
    const generated = writeCustomTarget({
      definitionPath: selection.targetDefinition,
      outDir,
...
function preparePartitions(app, config, buildDir, targetId) {
  ...
  // A board with a different flash size gets the table the app declared for it.
  const table = config.partitionsByTarget?.[targetId] || config.partitions
  if (!table) return plan({})
  if (typeof table === 'string') return plan({ csv: path.join(app.root, table) })
  ...
  if (app) {
    ...
    const partitions = preparePartitions(app, esp32Config, buildDir, selection.target)
  ...
  applySdkconfigPolicy(new Sdkconfig(sdkconfigFile, defaultsFile, appSdkconfigFile), {
    ...
    capabilities,
    bleOta,
    // An app's own table still wins; a composed board's is the fallback for the
    // apps that declare none, because the board is what knows its flash layout.
    partitionCsv: appPartitionCsv || boardPartitionCsv,
    boardConsole,
    boardPsram
  }).save()
...
export async function buildEsp32Firmware({ ctx, selection, app = null, env = ctx.env || process.env, bleOta = false, dryRun = false, verbose = false, stdout = console.log, stderr = console.error, configureOnly = false, output = '' }) {
  ...
  publishEsp32Output({ ctx, app, images: prepared.images, output, dryRun, stdout })
  return prepared
}

// The application image under a name the app chooses, so people handing the
// firmware on never have to dig through the build tree for gea_embedded.bin.
// --output (relative to the working directory) wins over the app's
// gea.targets.esp32.output (relative to the app).
export function resolveEsp32Output({ ctx, app, output = '' }) {
  if (output === true) fail('--output needs a file path, e.g. --output build/firmware.bin', ExitCode.usage)
  if (output) return path.resolve(ctx.cwd, output)
  const declared = app ? appTargetConfig(app, 'esp32').output : ''
  return declared ? path.join(app.root, declared) : ''
}

export function publishEsp32Output({ ctx, app, images, output = '', dryRun = false, stdout = console.log }) {
  const file = resolveEsp32Output({ ctx, app, output })
  if (!file) return ''
  if (dryRun) {
    stdout(`[dry-run] copy ${images.app} -> ${file}`)
    return file
  }
  if (!existsSync(images.app)) fail(`The build produced no application image at ${images.app}.`, ExitCode.buildFailed)
  mkdirSync(path.dirname(file), { recursive: true })
  copyFileSync(images.app, file)
  stdout(`Firmware image: ${path.relative(ctx.cwd, file) || file}`)
  return file
}
