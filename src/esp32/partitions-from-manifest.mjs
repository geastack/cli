...
const APP_SOURCE = 'package.json gea.targets.esp32.partitions'
const header = (source) => `# Generated from ${source} -- do not edit.\n# Name, Type, SubType, Offset, Size, Flags\n`
...
export function writePartitionTable({ table, appRoot, outDir, source = APP_SOURCE }) {
  const rows = []
  const payloads = []
  ...
  writeFileSync(file, header(source) + rows.join('\n') + '\n')
