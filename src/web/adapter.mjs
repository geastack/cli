...
// CLI. Every one of these scripts is spawned with the app's own folder as cwd
// and that is the only place the app directory is stated -- a root's own
// package.json is one of the places discovery looks, so an app outside the
// examples workspace resolves like any other. Same as the macOS adapter.
