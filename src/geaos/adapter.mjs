...
function geaosEnv(ctx, selection, env) {
  // geaos is the one target whose scripts do NOT run in the project: they run
  // in the geaos package's own target directory, because they reach for their
  // siblings there. So it is the one place that has to state the project, and
  // it states it under the name createContext already reads.
  const out = { ...env, GEA_BOARD_TARGET: selection.target, GEA_PROJECT_ROOT: ctx.projectRoot }
