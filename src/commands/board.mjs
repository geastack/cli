...
export async function buildCommand(ctx, parsed, rest, options) {
  ...
  switch (selection.adapter) {
    ...
    case 'esp32-idf': {
      await buildEsp32Firmware({ ctx, selection, app, env, bleOta: bleOtaRequested(parsed, app, env), dryRun: base.dryRun, verbose: flag(parsed, 'verbose'), stdout: base.stdout, stderr: base.stderr, configureOnly: flag(parsed, 'configure-only'), output: option(parsed, 'output', '') })
      return 0
    }
    case 'rp2350-pico':
      buildRp2350({ ctx, selection, app, env, dryRun: base.dryRun, stdout: base.stdout, stderr: base.stderr, configureOnly: flag(parsed, 'configure-only'), output: option(parsed, 'output', '') })
      return 0
    case 'geaos-linux':
    case 'geaos-arm64':
...
// the recorded address is then stale and only the cable reaches the board.
