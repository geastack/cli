...
test('createChildEnv exports exact installed package paths and nothing stale', (t) => {
  ...
  // No project-root variable: a child that needs the project is spawned WITH
  // the project as its cwd, and createContext resolves a project from cwd.
  // Announcing it in the environment too was a second spelling of the same
  // value that could disagree with the first.
  assert.equal(env.GEA_APPS_ROOT, undefined)
  assert.equal(env.GEA_PROJECT_ROOT, undefined)
...
test('targets.json entries resolve to installed target project directories', (t) => {
  ...
  // Resolved through the package it names, not through a path that walks out
  // of @geastack/targets and hopes for a neighbour.
  assert.equal(targets.geaos.package, '@geastack/geaos')
  assert.equal(targets.geaos.targetDir, path.join(fixture.installed('geaos'), 'targets/geaos'))
})
...
test('manifest helpers validate current app and installed target metadata', (t) => {
  ...
  // The meta line leads with the app's ABSOLUTE directory.
  assert.equal(appCmakeMeta(ctx, current), `${current.root};index.tsx;gea`)
  assert.equal(path.isAbsolute(appCmakeMeta(ctx, current).split(';')[0]), true)
...
test('a target entry both enables the target and configures its native build', (t) => {
  ...
  assert.equal(esp32.output, 'build/firmware.bin')
...
test('a board with different flash gets the table the app declared for it', (t) => {
  ...
  writeJson(path.join(root, 'package.json'), {
    ...
    gea: {
      ...
      targets: {
        esp32: {
          partitions: {
            ota_0: { type: 'app', subtype: 'ota_0', offset: '0x20000', size: '0x800000' },
            storage: { type: 'data', subtype: 'spiffs', offset: '0x1020000', size: '0xfe0000' }
          },
          partitionsByTarget: {
            'esp32-s3-devkit-n16r8': {
              ota_0: { type: 'app', subtype: 'ota_0', offset: '0x20000', size: '0x400000' },
              storage: { type: 'data', subtype: 'spiffs', offset: '0x820000', size: '0x7e0000' }
            },
            // Dropped: a key with no usable table must not shadow the default.
            'esp32-s3-nothing': {}
          }
        }
      }
  ...
  const esp32 = appTargetConfig(findCurrentApp(root), 'esp32')
  // The default table is untouched by the presence of an override.
  assert.equal(esp32.partitions.ota_0.size, '0x800000')
  assert.equal(esp32.partitionsByTarget['esp32-s3-devkit-n16r8'].ota_0.size, '0x400000')
  assert.equal(esp32.partitionsByTarget['esp32-s3-devkit-n16r8'].storage.offset, '0x820000')
  assert.ok(!('esp32-s3-nothing' in esp32.partitionsByTarget))
})

test('a per-target partition table is validated like the default one', (t) => {
  const { root } = createFixture(t)
  writeJson(path.join(root, 'package.json'), {
    name: 'pedal',
    gea: {
      ...
      targets: {
        esp32: {
          partitionsByTarget: {
            'esp32-s3-devkit-n16r8': { ota_0: { type: 'app', subtype: 'ota_0', offset: '0x20000' } }
          }
        }
      }
    }
  })
  const errors = validateApp(findCurrentApp(root))
  // A table that only applies to one board is still a table the build writes.
  assert.ok(errors.some((error) => error.includes('partitionsByTarget.esp32-s3-devkit-n16r8.ota_0 needs a size')), errors.join('; '))
})
