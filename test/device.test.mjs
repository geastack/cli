...
// The host half of the GEADEV I2CSCAN diagnostic. The device half lives in
// @geastack/targets (targets/esp32/services/device_control.cpp) and is
// asserted there; the two must agree on the wire strings, so each repo pins
// its own side.
test('geadev.i2cScan speaks the GEADEV I2CSCAN protocol the firmware implements', async () => {
  const written = []
  const replies = [
    'I (1234) gea: GEADEV:I2CSCAN BEGIN',
    'GEADEV:I2CSCAN ADDR 0x38',
    'GEADEV:I2CSCAN ADDR 0x51',
    'noise from another subsystem',
    'GEADEV:I2CSCAN END count=2',
  ]
  const device = {
    trace: false,
    stderr() {},
    writeLine: async (line) => void written.push(line),
    readLine: async () => replies.shift() ?? null,
    collect: SerialDevice.prototype.collect,
  }
  const out = await geadev.i2cScan(device)

  assert.deepEqual(written, ['GEADEV I2CSCAN'], 'the request line the firmware parses with tokenEquals(command, "I2CSCAN")')
  assert.equal(
    out,
    ['GEADEV:I2CSCAN BEGIN', 'GEADEV:I2CSCAN ADDR 0x38', 'GEADEV:I2CSCAN ADDR 0x51', 'GEADEV:I2CSCAN END count=2'].join('\n'),
