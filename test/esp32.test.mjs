...
test('the application image is copied to the output the app or the command names', (t) => {
  const fixture = createFixture(t)
  const ctx = createContext(parseArgs([]), {}, fixture.root)
  const buildDir = path.join(fixture.root, '.gea/build/board/app-builds/pedal')
  mkdirSync(buildDir, { recursive: true })
  const images = { app: path.join(buildDir, 'gea_embedded.bin') }
  writeFileSync(images.app, 'image')
  const app = { root: path.join(fixture.root, 'pedal'), targetConfig: { esp32: { output: 'build/pedalboard.bin' } } }
  const lines = []
  const declared = publishEsp32Output({ ctx, app, images, stdout: (line) => lines.push(line) })
  assert.equal(declared, path.join(fixture.root, 'pedal/build/pedalboard.bin'))
  assert.equal(readFileSync(declared, 'utf8'), 'image')
  assert.match(lines[0], /Firmware image: pedal\/build\/pedalboard\.bin/)
  // The command line wins, relative to where it was typed.
  const explicit = publishEsp32Output({ ctx, app, images, output: 'out/fw.bin', stdout: () => {} })
  assert.equal(readFileSync(path.join(fixture.root, 'out/fw.bin'), 'utf8'), 'image')
  assert.equal(explicit, path.join(fixture.root, 'out/fw.bin'))
  // Nothing declared, nothing copied; a dry run copies nothing either.
  assert.equal(publishEsp32Output({ ctx, app: { root: fixture.root, targetConfig: {} }, images, stdout: () => {} }), '')
  const dry = []
  publishEsp32Output({ ctx, app, images, output: 'dry/fw.bin', dryRun: true, stdout: (line) => dry.push(line) })
  assert.match(dry[0], /\[dry-run\] copy/)
  assert.throws(() => publishEsp32Output({ ctx, app, images, output: true, stdout: () => {} }), /--output needs a file path/)
})
